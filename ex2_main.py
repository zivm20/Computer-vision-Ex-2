...
def conv1Demo(signal:np.array=np.array([1.1, 1, 3, 4, 5, 6, 2, 1]),kernel:np.array=np.array([1, 2, 2, 1])):
    #signal = np.array([1, 2, 2, 1])
    #kernel = np.array([1, 1])

    sig_conv = conv1D(signal, kernel)
    numpy = np.convolve(signal, kernel, 'full')
    if np.array_equal(sig_conv,numpy) == False:
...
def conv2Demo():
    ...
    f, ax = plt.subplots(1, 3)
    ax[0].imshow(c_img*255,cmap='gray')
    ax[1].imshow((cv_img - c_img)*255,cmap='gray')
    ax[2].imshow(cv_img*255,cmap='gray')
    plt.show()
...
def derivDemo():
    ...
    v = np.array([[1, 0, -1]])
    X2 = cv2.filter2D(img, -1, v,borderType=cv2.BORDER_REPLICATE)
    Y2 = cv2.filter2D(img, -1, v.T,borderType=cv2.BORDER_REPLICATE)
    ori2 = np.arctan2(Y2, X2).astype(np.float64)
    mag2 = np.sqrt(X2 ** 2 + Y2 ** 2).astype(np.float64)
    ...
    print("MSE: {}".format(255 * MSE(ori2, ori)))
    print("MSE: {}".format(255 * MSE(mag2, mag)))
